...
namespace ReadHub.Libro.Application.Services
{
    public class LoanExpirationChecker : BackgroundService
    {
        ...
        // Revisa los préstamos vencidos y envía notificaciones por correo electrónico
        private async Task CheckExpiredLoans()
        {
            ...
            foreach (var loan in loans)
            {
                ...
                // 1) NO SE HACEN MULTAS SI EL PRÉSTAMO YA FUE DEVUELTO
                  
                bool loanFullyReturned =
                    loan.Items != null &&
                    loan.Items.All(i => i.Quantity == 0);

                if (loanFullyReturned)
                    continue;

                // 🚫 2) NO SE HACEN MULTAS SI YA PAGÓ TODAS LAS MULTAS
                var fines = await fineRepo.GetFinesByUser(loan.UserId);

                bool allFinesPaidForLoan =
                    fines.Any(f => f.LoanId == loan.Id) &&
                    fines.Where(f => f.LoanId == loan.Id).All(f => f.IsPaid);

                if (allFinesPaidForLoan)
                    continue;
    ...
    <div class='footer'>
        © {DateTime.UtcNow.Year} ReadHub 
                ...
                if (loan.ReturnDate < now)
                {
                    ...
                    // ⭐⭐⭐ PRIMERA MULTA A PARTIR DEL DÍA 2 ⭐⭐⭐
                    if (daysLate < 2)
                        continue;
...
<div class='container'>
    ...
    <div class='footer'>
        ...
        //Busca el correo del usuario en el servicio de autenticación
