...
namespace ReadHub.Libro.Application.Services
{
    public class PendingLoanService
    {
        ...
        public PendingLoanService(
            IPendingLoanRepository pendingLoanRepo,
        ...
        public async Task CreatePendingLoanAsync(CreatePendingLoanDto dto)
        {
            ...
            var userLoans = await _loanRepo.GetUserLoansAsync(dto.UserId);

            if (UserHasOverdueLoans(userLoans))
                throw new InvalidOperationException("No puedes solicitar libros porque tienes préstamos vencidos.");
        ...
        //   BIBLIOTECARIO APRUEBA LA SOLICITUD
        public async Task ApprovePendingLoanAsync(Guid id, int approvedQuantity)
        {
            ...
            var userLoans = await _loanRepo.GetUserLoansAsync(pending.UserId);
            if (UserHasOverdueLoans(userLoans))
                throw new InvalidOperationException("No se puede aprobar esta solicitud porque el usuario tiene préstamos vencidos.");
        ...
        //   BIBLIOTECARIO RECHAZA LA SOLICITUD
        ...
        private bool UserHasOverdueLoans(IEnumerable<Loan> loans)
        {
            ...
            return loans.Any(loan =>
                ...
                loan.Items.Any(item => !item.IsReturned) 
            );
        }
    }
}
